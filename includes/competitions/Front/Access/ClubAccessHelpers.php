...
if ( ! function_exists( 'ufsc_lc_get_club_status_clause' ) ) {
	...
	function ufsc_lc_get_club_status_clause( string $table, array $columns ): string {
		if ( in_array( 'statut', $columns, true ) ) {
			return " AND LOWER(COALESCE({$table}.statut, '')) = 'actif'";
		}

		if ( in_array( 'status', $columns, true ) ) {
			return " AND LOWER(COALESCE({$table}.status, '')) IN ('actif','active')";
		}
...
if ( ! function_exists( 'ufsc_sync_user_club_link' ) ) {
	 ...
	 * @return void
	 */
	function ufsc_sync_user_club_link( int $club_id ): void {
		global $wpdb;
		...
		$new_user_id = absint( $row->responsable_user_id ?? 0 );
		if ( $new_user_id <= 0 ) {
			return;
		}
		...
		foreach ( (array) $linked_user_ids as $linked_user_id ) {
			...
			delete_transient( ufsc_lc_user_club_transient_key( $linked_user_id ) );
		}

		delete_transient( ufsc_lc_user_club_transient_key( $new_user_id ) );
	}
}
...
add_action(
	...
	static function( $user_login, $user ) {
		...
		$club_id = ufsc_get_current_club_id( $user_id );
		if ( $club_id > 0 ) {
			ufsc_sync_user_club_link( $club_id );
		}
	},
	10,
	2
...
add_action(
	...
	static function( int $club_id, int $old_user_id, int $new_user_id ) {
		...
		if ( $old_user_id > 0 && (int) get_user_meta( $old_user_id, 'ufsc_club_id', true ) === $club_id ) {
			...
			delete_transient( ufsc_lc_user_club_transient_key( $old_user_id ) );
		}
