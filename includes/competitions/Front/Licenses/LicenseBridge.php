...
class LicenseBridge {
	...
	public function search( string $term, int $club_id, string $license_number = '', string $birthdate = '' ): array {
		...
		if ( '' !== $status_column ) {
			$invalid_statuses = function_exists( 'ufsc_lc_get_excluded_licence_statuses' )
			...
			$invalid_statuses = array_values( array_filter( array_map( 'sanitize_key', (array) $invalid_statuses ) ) );
