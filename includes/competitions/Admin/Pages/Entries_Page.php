...
class Entries_Page {
	...
	public function ajax_search_licence() {
		...
		if ( '' !== $schema['status_expr'] ) {
			$excluded_statuses = function_exists( 'ufsc_lc_get_excluded_licence_statuses' )
				? ufsc_lc_get_excluded_licence_statuses()
				: array( 'inactive', 'inactif', 'invalide', 'invalid', 'expired', 'expire', 'desactive', 'suspended', 'suspendu', 'cancelled', 'annule', 'deleted', 'supprime', 'rejected', 'refused', 'refuse', 'blocked', 'bloque' );
			$excluded_statuses = array_values( array_filter( array_map( 'sanitize_key', (array) $excluded_statuses ) ) );
