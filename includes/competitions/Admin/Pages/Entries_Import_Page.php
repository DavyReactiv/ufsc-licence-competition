	...
	private function import_rows( $competition, array $rows ): array {
		...
		foreach ( $rows as $row ) {
			...
			if ( $licensee_id && $this->entry_repository->get_by_competition_licensee( $competition_id, $licensee_id ) ) {
				$report['skipped']++;
				$report['rows'][] = array( 'line' => $line, 'status' => 'ignored', 'message' => __( 'Doublon détecté (ligne ignorée).', 'ufsc-licence-competition' ) );
				continue;
			}
			...
			if ( ! $entry_id ) {
				$report['skipped']++;
				$report['rows'][] = array( 'line' => $line, 'status' => 'error', 'message' => __( 'Erreur lors de l’enregistrement en base.', 'ufsc-licence-competition' ) );
				continue;
			}
