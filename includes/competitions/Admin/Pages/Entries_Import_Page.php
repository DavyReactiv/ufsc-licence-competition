...
class Entries_Import_Page {
	...
	public function handle_import(): void {
		...
		$report = $this->import_rows( $competition, $parsed['headers'], $parsed['rows'] );
	...
	private function import_rows( $competition, array $headers, array $rows ): array {
		$report = array(
			'total' => count( $rows ),
			'imported' => 0,
			'skipped' => 0,
		...
		foreach ( $rows as $row ) {
			...
			if ( $require_valid_license && ! $licensee_id && '' === trim( $license_number ) ) {
				$report['skipped']++;
